...
export const LayerDropdown: React.FC = () => {
  const { activeViewMode: viewMode, dimension } = useMission()
  const { setViewMode: onViewModeChange, setDimension: onDimensionChange } =
    useMissionActions()
  ...
  return (
    // Placement travels with the feature: App.tsx used to wrap this in
    // .map-overlay-top-right, and the canvas slot turns pointer events off so
    // an empty overlay layer cannot swallow clicks meant for the map.
    <div className="map-overlay-top-right" style={{ pointerEvents: 'auto' }}>

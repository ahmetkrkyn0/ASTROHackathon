...
| `POST` | `/api/reachable` | Energy-reachability isochrones: where this rover can get from here, now, on this charge -- and where it could get if it left N hours later. A forward sweep over a time-expanded grid carrying charge as a STATE, not a cost: on Site11 at a lit epoch 28.94% of drive edges and 53.9% of wait edges are negative (the array outproduces the load), so a Dijkstra over energy is unsound -- and at a fully dark epoch exactly 0% are, which is how one would pass a test suite and be wrong in operation. The set CONTAINS what /api/plan-4d accepts, so the safe reading is the negative one, and `energy_binds` says when the frontier was the clock rather than the battery. |
